...
class KittiDataset(Dataset):
    ...
    def __init__(self, lidar_dir, rgb_dir,color_dir, intensity_dir,incidence_dir,binary_dir,label_dir,reflectance_dir,rgb_transform=None,
    ...
    def __getitem__(self, index):
        ...
        #concatenated_img = torch.cat((binary,lidar), dim=0) #T10 #in_channels=2
        #concatenated_img = torch.cat((binary,lidar,reflectance), dim=0) #T9 #in_channels=3
        #concatenated_img = torch.cat((binary,lidar, color, rgb), dim=0) #T8 #in_channels=6 
        #concatenated_img = torch.cat((binary,lidar,reflectance, color, rgb), dim=0) #T7 #in_channels=7

        #concatenated_img = torch.cat((binary, label,lidar), dim=0) #T6#in_channels=3
        #concatenated_img = torch.cat((binary, label,reflectance,lidar), dim=0) #T5#in_channels=4
        #concatenated_img = torch.cat((binary, label,incidence,lidar), dim=0) #T4 #in_channels=4
        concatenated_img = torch.cat((binary, label,incidence,reflectance,lidar), dim=0) #T3 #in_channels=5
        #concatenated_img = torch.cat((binary, label,incidence,lidar, color, rgb), dim=0) #T2 #in_channels=8
        #concatenated_img = torch.cat((binary, label,incidence,reflectance,lidar, color, rgb), dim=0) #T1 #in_channels=9
